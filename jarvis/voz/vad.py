...
def _resample_to_target(audio_bytes: bytes, sample_rate: int, target_rate: int) -> bytes:
    if np is None:
        return b""
...
class _SimpleAec:
    def __init__(self, reference: _PlaybackReference, sample_rate: int, frame_ms: int) -> None:
...
def _get_aec_processor(sample_rate: int, frame_ms: int) -> _SimpleAec | None:
    ...
    if backend != "simple" or np is None or sample_rate != 16000:
        _AEC_PROCESSOR = None
        _AEC_KEY = None
        return None
    key = (
...
def apply_aec_to_audio(audio_bytes: bytes, sample_rate: int, frame_ms: int = 30) -> bytes:
...
class VoiceActivityDetector:
    ...
    def __init__(
        ...
        self._agc_target_rms = max(
            0.0, _env_float("JARVIS_AUDIO_AGC_TARGET_RMS", 0.06)
        )
    ...
    def _preprocess_frame(self, audio_frame: bytes) -> bytes:
        ...
        if self._ns_gate_rms > 0.0 and rms < self._ns_gate_rms:
...
class StreamingVAD:
    ...
    def __init__(
        ...
        self.device = device
        self._metrics_enabled = os.environ.get("JARVIS_VAD_METRICS", "").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
