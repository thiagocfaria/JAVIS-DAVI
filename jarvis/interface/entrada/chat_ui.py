...
from ..infra.chat_inbox import append_line
...
def _setup_watchdog(root: tk.Tk, log_path: Path, on_change) -> Callable[[], None] | None:
...
def main() -> int:
    ...
    parser.add_argument(
        "--tail", type=int, default=200, help="linhas do log para mostrar"
    )
    parser.add_argument(
        "--poll-ms", type=int, default=800, help="intervalo de atualizacao"
    )
    ...
    log_path = Path(
        args.log_path
        or os.environ.get(
            "JARVIS_CHAT_LOG_PATH", str(Path.home() / ".jarvis" / "chat.log")
        )
    )
    inbox_path = Path(
        args.inbox_path
        or os.environ.get(
            "JARVIS_CHAT_INBOX_PATH", str(Path.home() / ".jarvis" / "chat_inbox.txt")
        )
    )
