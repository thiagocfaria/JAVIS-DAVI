...
class ChatShortcut:
    ...
    def __init__(
        ...
        self._listener: object | None = (
            None  # keyboard.Listener quando pynput disponível
        )
        self._running = False
        ...
        self._file_trigger_path = (
            file_trigger_path or os.environ.get("JARVIS_CHAT_SHORTCUT_FILE", "").strip()
        ) or None
    ...
    def _default_chat_command(self) -> str:
        ...
        module = "jarvis.interface.entrada.chat_ui"
            ...
            if sys.platform.startswith("linux") and _is_wayland() and not _has_x11():
                ...
                try:
                    if _keyboard is None:
                        self._last_error = "pynput_not_available"
                    else:
    ...
    def stop(self) -> None:
        ...
        if self._listener:
            try:
                stop_method = getattr(self._listener, "stop", None)
    ...
    def is_running(self) -> bool:
        ...
        listener_running = self._listener is not None and getattr(
